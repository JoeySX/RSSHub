...
module.exports = async (ctx) => {
    ...
    // 取消search=today
    const res = await got_ins.get(url.resolve(base, `${section}${ctx.params.id}${typefilter}`));
    ...
    list = $('.tr3', list).not('.tr2.tac').nextAll().get(); // 截取前50条
    ...
    const out = await Promise.all(
        list.map(async (item) => {
            ...
            // 解析点赞数
            const liked = $('span').eq(0).text();
            // console.log('title: ' + title.text())
            // console.log(' liked: ' + liked);
            // 过滤掉点赞数小于100的帖子
            const likedInt = parseInt(liked);
            // console.log(likedInt + ' ' + title.text());
            if (likedInt < 100) {
            ...
            const single = {
                title: `[${liked}] ${catalog} ${title}`, // 添加点赞数到标题
                link,
                guid: path,
            };
